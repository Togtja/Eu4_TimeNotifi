...
void create_memory_class_for_eu4(std::unique_ptr<CrossMemory>& mem, const std::string& excecutable) {
    try {
        mem.reset(new CrossMemory(excecutable));
    }
...
auto get_reapeating(bool clear_repeat) {
    ...
    if (ImGui::BeginPopup("Load Noti Popup")) {
        ...
        if (ImGui::Button("Load", {200, 20})) {
            ...
            for (size_t i = 0; i < amount_of_notify; i++) {
                ...
                ss >> date_day >> date_month >> date_year >> repeat >> repeat_day >> repeat_month >> repeat_year >> std::ws;
                // Get remainder of ss
